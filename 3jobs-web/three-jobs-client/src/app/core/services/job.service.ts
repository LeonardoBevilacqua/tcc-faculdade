...
export class JobService extends EntityService<Job> {
    ...
    public search(title: string, page = 0, size = 10, city = '', jobRole = '' ) {
        return this.httpClient.get(`${this.apiUrl}/${this.endpoint}?title=${title}&page=${page}&size=${size}&city=${city}&jobRole=${jobRole}`);
    }
